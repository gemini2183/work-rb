...
### Клиент-специфичные скрипты

```
python metrika_stats_ege_merlin.py --date-from 2026-06-01 --date-to 2026-07-17
```

Для ЕГЭ Merlin — поверх сбора `metrika_stats.py` фильтрует только рекламный
трафик, схлопывает Source в YaDirect и исключает letnie по UTM Content (1:1
логика прод-job `ym2_merlin_ege`). Результат — уже готовые для сверки с
Директом данные, без ручного повторения фильтрации при каждом анализе.

```
python reconcile_ege_merlin.py --date-from 2026-06-01 --date-to 2026-07-17
```

Считает сверку по кампаниям (Директ: форма+звонок vs Метрика.форма+
Callibri.звонок) — реализация методологии
`База_знаний/Паттерны/Сверка-конверсий-Директ-Метрика-Коллтрекинг.md` для
этого клиента. Не собирает данные сам — сначала нужно прогнать
`direct_stats.py` (с `--goals 222490461,40404508`), `metrika_stats_ege_merlin.py`
и `callibri_stats.py` за тот же период. Результат — таблица по кампаниям и
CSV в `Статистика/reconcile_<период>.csv`; звонки без UTM показываются отдельно
как неразбиваемый остаток (см. методологию — это системное ограничение, не
временный пробел).
